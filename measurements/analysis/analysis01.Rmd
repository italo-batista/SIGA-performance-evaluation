...
ggplot(data = post.all, aes(x=as.factor(RAM), y=log10(TIMESTAMP), colour=as.factor(REQ_MIN))) +
  geom_point(size=1, alpha=0.4) + ylim(1,2) #coloquei o lim pra tentar tirar os outliers da visualização

ggplot(data = tests, aes(x=as.factor(REQ_MIN), y=TIMESTAMP, colour=as.factor(RAM))) +
  geom_point(size=1, alpha=0.4) + ylim(0,200) #coloquei o lim pra tentar tirar os outliers da visualização
```
